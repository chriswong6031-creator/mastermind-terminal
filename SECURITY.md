...
| --- | --- | --- |
| **Security headers on every app route** — CSP (incl. `frame-ancestors 'self'`), `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Opener-Policy`, HSTS | `terminal/next.config.ts` | Competitors iframing/rehosting the UI (clickjacking/embedding); MIME confusion; referrer leakage. CSP is a defence-in-depth allowlist for scripts/styles/connections. |
