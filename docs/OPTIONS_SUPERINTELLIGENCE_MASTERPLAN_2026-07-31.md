...
| R5 IA restructure (7 categories) + blind-spot surfaces (0DTE dash, largest-trades, chain browser, P/C history, exp-vs-realized, export) | 🔓 OPEN — first slice IN FLIGHT 08-03: §5.3 PRISM→Exposure merge (branch `claude/prism-exposure-merge-20260803`; retire PRISM tab, shared sign-correct matrix renderer, port Confluence + HeatSeeker, `?tab=prism` alias). **PR #164 RESOLVED 08-03: closed as superseded** — master already ships its flat-sidebar intent (AppNav 7-item, next.config redirects) and resolves the one design conflict (Leaders/Radar → Discover) in R5's direction; nothing to re-implement; salvage check pending: multi-watchlist pill switcher on PortfolioView. **Tape CSV export ✅ DONE 08-10 — PR #380** — RFC-4180 export covers the complete filtered/sorted collection rather than the 150-row DOM window, preserves `~` inferred direction and `display_only` authority metadata. **Screener active-preset CSV export ✅ DONE 08-10 — PR #382** — the six existing views share one filtered/sorted selector path with the UI; the v1 artifact records source cadence/as-of, armed filters, exact order, `display_only` authority, and view-specific interpretation limits. **Seven-category IA Stage A ✅ DONE 08-10 — PR #383** — presentation/routing only: Command · Flow · Exposure · Structure · Volatility · Statistics · Prophet over the existing panes; the unbuilt Statistics publisher remains explicitly gated with no synthetic values. **0DTE + largest-event Flow boards ✅ DONE 08-10 — PR #384** — one responsive selector/pane contract over immutable `live_flow.feed/v1`; the publisher's 0DTE flag and raw aggregate premium are the only derivations. Feed rows coalesce one contract's poll-batch prints, so the honest surface says “Largest Events,” retains `~` direction, and does not claim individual trades, NBBO tiers, P&L, or opening/closing intent. Chain browser, P/C history, and expected-vs-realized remain open. |
| R5 P/C OI history | 🟡 IN FLIGHT 08-10 — branch `claude/options-put-call-history-r5-20260810`; responsive display-only chart over the existing nightly `options_hub.oi_time/v1` fetch, deriving only `put_oi / call_oi`, retaining t-1 provenance, and leaving invalid denominators as gaps. No new fetch, API, producer, or authority. |
