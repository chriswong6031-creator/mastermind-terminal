...
| Wave / item | Status (verified 2026-08-10) |
|---|---|
| R0.1 chain-heat relight | ✅ DONE 07-31 — relit on M1, artifact verified current on public R2 |
| R0 repair wave (seal, cadence, schema v2, two-tier, U-CHAIN, dead-man) | 🟡 PARTIAL LIVE — macro #4152 (+ #4222) shipped the repair plumbing; live `meta.json` at 2026-08-10T19:17Z verifies `delta_mode: time_window` + `two_tier: true`, but `cadence_sec_measured/cycle_sec: 4939.4` still misses the 120 s target. Chain heat is current; the cadence performance exit remains OPEN. |
...
| R5 IA restructure (7 categories) + blind-spot surfaces (0DTE dash, largest-trades, chain browser, P/C history, exp-vs-realized, export) | 🟡 FIVE SLICES LIVE / TWO CORE CONTRACT-GATED — §5.3 PRISM→Exposure shipped in #344 and PR #164 was closed as superseded. **Tape CSV ✅ #380** (full filtered/sorted set, `~` + `display_only` retained); **Screener active-preset CSV ✅ #382** (one UI/export selector, source/filter/order/authority receipts); **seven-category IA Stage A ✅ #383** (presentation/routing only); **0DTE + Largest Events ✅ #384** (only publisher 0DTE + aggregate-premium derivations; no individual-trade/NBBO/P&L/open-close claims); **P/C OI history ✅ #385** (nightly t-1 display-only ratio with invalid-denominator gaps). All five are merged and live on 08-10. **Chain browser GATED:** #5259 is disabled and publishes profile subsets, not a complete chain. **Expected-vs-realized GATED:** `options_hub.moves/v1` has a current expected band plus aggregate calibration, not per-session expected/realized history. **Portfolio multi-watchlist salvage IN FLIGHT 08-10** — branch `claude/options-portfolio-watchlists-r5-20260810`; existing Supabase + `mm.wls` only, no new API/schema/authority. |
...
| R6 Prophet superintelligence (spine, multi-lane origination, live re-score, distribution) | 🟡 WAVE 0 + R6.2-A LIVE; OUTCOME PIPELINE CODE-ONLY — terminal #371 ships governed `options.prophet_shadow/v1`; #373 ships private operator-reviewed `options.issue_desk/v1`, with every authority flag false and no public Issue Desk artifact/brokerage action. Macro #5211 merges immutable episode/session-outcome/H+60 scaffolding, but current `main` has zero committed `data/options_signal_episode/` rows. Its preregistration forbids campaign/model work until an RTH stage-before-index publication and later nightly produce nonempty causally ordered episode/H+60 receipts. Campaigns, sparse selector, exact contract optimizer, lifecycle, Today/Pulse, and `options.model_portfolio/v1` remain evidence-gated. |
