...
| Wave / item | Status (verified 2026-08-11) |
|---|---|
| R0.1 chain-heat relight | ✅ DONE 07-31 — relit on M1, artifact verified current on public R2 |
| R0 repair wave (seal, cadence, schema v2, two-tier, U-CHAIN, dead-man) | 🟡 PARTIAL LIVE — macro #4152 (+ #4222) repair plumbing is live; `meta.json` at 2026-08-10T19:17Z verified `delta_mode: time_window` + `two_tier: true`, but `cadence_sec_measured/cycle_sec: 4939.4` still misses the 120 s target. **MERGED / CODE-COMPLETE + DEPLOYED:** macro #5273 (`7c550326…`) bounded prior-OI memory; scheduler repair #5349 was squash-merged as `4e9bbb353375e388af152b439891dc692638defd` and deployed to the dedicated clean `/Users/chriswong/chainsnap-ops-wt` at that exact HEAD. The installed plist points its executable, working directory and `PYTHONPATH` at that checkout; `data/chain_snapshots` is an exact symlink to `/Users/chriswong/flow-ops-wt/data/chain_snapshots` (physical/link device+inode `16777234:292244470`). Pre/post state manifests were byte-identical at 2,103 rows / 202,960 bytes / SHA-256 `0a097cace4dd4efc58bcb91ba745212519e5f4c390e093e9dae75b1b270d5d79`; after 155 seconds the launchd label remained inert (`runs=1`, exit 0, no PID). U-CHAIN authority remains disabled. **NEXT LIVE-EVIDENCE GATE:** the next untouched 06:30 PT M1 RTH schedule must show all six RSS phases, material memory improvement, governed receipt/stage-before-index publication, and no writer-lock or latency regression. |
...
| R6 Prophet superintelligence (spine, multi-lane origination, live re-score, distribution) | 🟡 WAVE 0 + R6.2-A LIVE; PIT FOUNDATION + CANONICAL CAMPAIGN PUBLISHER MERGED / DEPLOYED; SCHEDULED CAMPAIGN ARTIFACT AND PROMOTION GATED — terminal #371 serves governed `options.prophet_shadow/v1` and #373 the private operator-reviewed `options.issue_desk/v1`; all automatic authority flags remain false. **MERGED / CODE-COMPLETE:** macro #5211 (`cadd31f…`) establishes immutable episode/H+60/session-outcome contracts; #5324 (`2a526d…`) durably lands the exact episode, H+60, session-outcome, and checkpoint blobs on `main`. **LIVE-EVIDENCE-PROVEN:** the 2026-08-10 RTH publication carried 384 decision + 384 availability rows (336,740 bytes; SHA-256 `fbc7f802cd6f999fdc600f25acccfe66a2f220de491b83b1d1d4ddebc0e8dd7f`), stage-before-index, strict pairing/clocks, and WAL=0; scheduled nightly run `31440972065` / engine job `93655174194` produced 384 episodes, H+60 301 total / 261 complete, session outcomes 270 total / 261 complete, zero rejects. All rows remain research-only and training-ineligible. **FAIL-CLOSED PUBLICATION SHIPPED:** #5338 final head `f2f892925c24ef50886d9a7f84932fee53a1f53c` passed all 12 exact-head CI packs (run `31509875496`) plus fences (`31509875379`), squash-merged as `4fb43412fdf82fb152947499fead87aced59ba43`, and production was verified at a current-`main` descendant. **LEGACY ONLY:** #5332 (`a77c565…`) remains an eight-row retrospective threshold cohort, not canonical campaign aggregation. **CANONICAL CAMPAIGN PUBLISHER SHIPPED:** #5362 final head `ba4119fa17ea08f525fcf147867ffcf267aa6901` passed all 12 exact-head CI packs (run `31535007985`) plus fences (`31535007753`), squash-merged as `d8e290032710d84e538c32af0d58358a16407c88`, and production checkout was verified at that exact merge. It installs versioned campaign-revision, outcome and checkpoint publication with the repaired effective prospective boundary `2026-08-12T13:30:00Z`. **NO SCHEDULED CAMPAIGN ARTIFACT HAS YET BEEN OBSERVED:** merge/deploy and retrospective dry-run rows are not prospective accrual. Selector, optimizer, lifecycle, Today/Pulse, rank, sizing, trading and training promotion remain closed until a normal nightly publishes the canonical artifacts and genuinely post-boundary prospective rows satisfy their preregistered gates. |
...
Macro #5362 deploys the canonical versioned campaign-revision, outcome and checkpoint writer. Its repaired effective prospective boundary is `2026-08-12T13:30:00Z`; no normally scheduled campaign artifact or genuinely post-boundary campaign row has yet been observed. Until that evidence accrues and passes the preregistered gate, the writer remains research-only and cannot feed selection, optimization, lifecycle, Today/Pulse, Macro rank, sizing, trading or training.
