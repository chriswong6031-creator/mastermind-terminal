...
| A20.28 | Resting-detent sheets leave the chart **undimmed** | `IMG_2366`'s chart band above the 62 % Drawings sheet is at full ink, and §1.6's `tvScrim` is measured only above a **raised** sheet (top edge y = 62 pt = `.large`) — SwiftUI's default partial-detent scrim is therefore a parity defect. Drawings and the Analysis hub set `.presentationBackgroundInteraction(.enabled(upThrough:))` at their resting fraction (0.62 / 0.6): the chart stays live and crisp, and the system dim returns only at `.large`. Known trade: with background interaction enabled the OS routes outside taps to the chart instead of dismissing — TV's own behavior (the live chart is the point); drag-down and the ✕ remain the dismissals. Web twin: terminal PR #324 (scrim transparent at the 62 %/60 % detents, dim at 96 %) |
