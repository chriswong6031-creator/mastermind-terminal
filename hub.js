...
//   (b) US: wss://delayed.polygon.io/stocks AM.* dynamic per-symbol subs, LRU 500, chg vs manifest
//
...
const store = new Store(MANIFEST_PATH);
...
function handleQuotes(res, url) {
  ...
  if (!DISABLE_US && polygon && polygon.isHealthy()) {
    for (const sym of syms) {
      if (classify(sym) === "us") polygon.ensureSubscribed(sym);
    }
  }
...
function boot() {
  ...
  // Background intervals: prune stale entries + supervise crypto failover.
