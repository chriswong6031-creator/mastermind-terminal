...
def build_ratios(vrow, frow, annual) -> dict:
    ...
    if frow:
        # UNIT RULING: Tushare fina_indicator margins/roe are PERCENT-form (91.1796 == 91.1796%).
        # The contract stores them as 0..1 FRACTIONS (same as gen_fund_us grossMargins) → divide by 100.
        def _pct_frac(v):
            v = _f(v)
            return v / 100.0 if v is not None else None
        gms = frow.get("gross_margin_series")
        cur["gross_margin"] = _pct_frac(frow.get("gross_margin")) if frow.get("gross_margin") is not None else (
            _pct_frac(gms[-1]) if isinstance(gms, (list, tuple)) and len(gms) else None)
        cur["net_margin"] = _pct_frac(frow.get("net_margin"))
        cur["roe"] = _pct_frac(frow.get("roe"))
...
def build_ownership(holders: list) -> dict:
    ...
    for h in (holders or [])[:10]:
        # UNIT RULING: Tushare top10_floatholders `ratio` is PERCENT form (600519 top holder 54.4038).
        # The fund.json contract stores top_inst[].pct as a 0..1 FRACTION (US emitter passes yfinance
        # pctHeld through as a fraction). Always divide the percent-form by 100.
        pct = _f(h.get("ratio"))
        if pct is not None:
            pct = pct / 100.0
        top.append({"name": h.get("name"), "pct": pct, "value": None})
