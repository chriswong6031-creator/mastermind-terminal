...
def build_ratios(yf: dict, annual) -> dict:
    cur = {"pe_ttm": yf.get("trailing_pe"), "pe_fwd": yf.get("forward_pe"), "ps": yf.get("ps"),
           ...
           # UNIT RULING: yfinance margins/roe/roa are already 0..1 decimals (0.5645 == 56.45%) and the
           # contract stores them as 0..1 FRACTIONS (same as gen_fund_us grossMargins) → pass through raw.
           "gross_margin": _f(yf.get("gross_margin")), "net_margin": _f(yf.get("net_margin")),
           "roe": _f(yf.get("roe")), "roa": _f(yf.get("roa")),
