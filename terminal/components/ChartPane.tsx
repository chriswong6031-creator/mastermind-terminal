...
import ChartPanel, { type DetectCmd, type PineScript } from "@/components/ChartPanel";
...
export default function ChartPane({ idx, symbol, isActive, onActivate, row, tf, chartType, inds, tool, detectCmd, compare, magnet, replayIdx, onMeta, drawings, onDrawingsChange, pineScript, onPineResult, indParams, hidden, onToggleHidden, onRemoveInd, onOpenSettings, onOpenSource }:
  { idx: number; symbol: string; isActive: boolean; onActivate: (i: number) => void; row?: { col?: string; last?: number; chg?: number } | null; tf: string; chartType: string; inds: Set<string>; tool: string | null; detectCmd: DetectCmd; compare: string[]; magnet: boolean; replayIdx: number | null; onMeta: (m: { total: number }) => void; drawings: Drawing[]; onDrawingsChange: (d: Drawing[]) => void; pineScript?: PineScript | null; onPineResult?: (r: { ok: boolean; error?: string; plots: number; shapes: number } | null) => void;
  ...
  return (
    <div className={`pane${isActive ? " on" : ""}`} onPointerDownCapture={() => { if (!isActive) onActivate(idx); }}>
      ...
      <ChartPanel symbol={symbol} chartType={chartType} indicators={inds} timeframe={tf} replayIdx={isActive ? replayIdx : null} onMeta={isActive ? onMeta : undefined} tool={isActive ? tool : null} drawings={merged} onDrawingsChange={handleChange} detectCmd={isActive ? detectCmd : null} compare={isActive ? compare.filter((c) => c !== symbol) : []} magnet={isActive ? magnet : false} isActive={isActive} syncId={idx} pineScript={pineScript} onPineResult={isActive ? onPineResult : undefined} indParams={indParams} hidden={hidden} onToggleHidden={onToggleHidden} onRemoveInd={onRemoveInd} onOpenSettings={onOpenSettings} onOpenSource={onOpenSource} key={symbol + tf + chartType} />
    </div>
  );
}
