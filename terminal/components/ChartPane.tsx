...
export default function ChartPane({ idx, symbol, isActive, onActivate, row, tf, chartType, inds, tool, detectCmd, compare, magnet, replayIdx, onMeta }:
  ...
  useEffect(() => { let alive = true; setDrawings([]); fetch(`/api/drawings?symbol=${symbol}`).then((r) => r.json()).then((d) => { if (alive) setDrawings(d.drawings || []); }).catch(() => {}); return () => { alive = false; flush(); }; }, [symbol, flush]);
  ...
  return (
    <div className={`pane${isActive ? " on" : ""}`} onPointerDownCapture={() => onActivate(idx)}>
      ...
      <ChartPanel symbol={symbol} chartType={chartType} indicators={inds} timeframe={tf} replayIdx={isActive ? replayIdx : null} onMeta={isActive ? onMeta : undefined} tool={isActive ? tool : null} drawings={drawings} onDrawingsChange={onChange} detectCmd={isActive ? detectCmd : null} compare={isActive ? compare.filter((c) => c !== symbol) : []} magnet={isActive ? magnet : false} key={symbol + tf + chartType} />
    </div>
  );
}
