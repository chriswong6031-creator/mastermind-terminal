...
type Row = { sym: string; name: string; zh?: string; sec: string; col: string; last: number; chg: number; vol: number; verdict: string | null; wr: number | null; pf: number | null; cagr: number | null; regimeBull: boolean | null };
  ...
  return (
    <div className="app2">
      <header className="topbar">
        ...
        <span className="scr-stat"><span><b>{view.length}</b> {t("matches")}</span><span><b className="up">{nBuy}</b> {t("buyLc")}</span><span><b className="down">{nSell}</b> {t("sellLc")}</span><span>{t("avgWR")} <b>{avgWr.toFixed(0)}%</b></span></span>
      ...
      <main className="main2">
        <div className="scr-filters">
          ...
          <button className={`chip${verdict === "all" ? " on" : ""}`} onClick={() => setVerdict("all")}>{t("anySignal")}</button>
          ...
          <button className={`chip${uptrend ? " on" : ""}`} onClick={() => setUptrend((u) => !u)}>{t("uptrendRegime")}</button>
          <button className={`chip${sec === "Equities" ? " on" : ""}`} onClick={() => setSec((s) => s === "Equities" ? "all" : "Equities")}>{t("equities")}</button>
          <button className={`chip${sec === "Crypto" ? " on" : ""}`} onClick={() => setSec((s) => s === "Crypto" ? "all" : "Crypto")}>{t("crypto")}</button>
          <span style={{ marginLeft: "auto", color: "var(--text-dim)", fontSize: 11 }}>{t("scanAsOf").replace("{d}", asOf)}</span>
        </div>
        <div className="scr-table">
          <table className="scr">
            <thead><tr>
              {th("sym", t("symbol"))}{th("last", t("colLast"))}{th("chg", t("colChgPctShort"))}{th("verdict", t("signalCol"))}
              <th onClick={() => setSort((s) => ({ k: "regimeBull", dir: s.k === "regimeBull" && s.dir === -1 ? 1 : -1 }))} className={sort.k === "regimeBull" ? "sorted" : ""}>{t("regime")}</th>
              {th("wr", t("winRate"))}{th("pf", t("profitFactor"))}{th("cagr", t("cagr"))}{th("vol", t("colVolume"))}
            </tr></thead>
            <tbody>
              {view.length === 0 && (
                <tr className="empty-row"><td colSpan={9} style={{ textAlign: "center", color: "var(--muted)", padding: "44px 16px", fontSize: 13 }}>
                  {!loaded ? t("loadingScan") : err ? t("scanLoadErr") : t("noFilterMatch")}
                </td></tr>
              )}
      ...
      <div className="ticker"><span className="lbl">{t("goldenOracleScan")}</span><span style={{ color: "var(--text-2)" }}>{t("scanFootClick").replace("{nBuy}", String(nBuy)).replace("{nSell}", String(nSell)).replace("{n}", String(rows.length))}</span></div>
    </div>
  );
}
