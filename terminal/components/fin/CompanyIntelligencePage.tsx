          ...
          {lens === "topics" && (
            <section className="ci-lens-panel">
              ...
              {activeContext.topics.timeline.length ? (
                <ul className="ci-topic-list">{activeContext.topics.timeline.map((topic) => <li key={topic.tag}><span className={`ci-topic-status ${topic.status}`} aria-hidden /><div><strong>{topicTagLabel(topic.tag, zh)}</strong><small>{topic.first_event_id} → {topic.last_event_id}</small></div><span className="fin-tag" style={{ "--c": topic.status === "added" ? "var(--up)" : topic.status === "dropped" ? "var(--down)" : "var(--brand-2)" } as React.CSSProperties}>{topicStateLabel(topic.status, zh)}</span><b className="num">{topic.event_count}</b></li>)}</ul>
              ) : (
