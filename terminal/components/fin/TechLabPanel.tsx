...
/** Family label → display name. */
function familyLabel(family: string | null | undefined): string {
  if (!family) return "";
  const map: Record<string, string> = {
    ma_crosses: "MA Cross",
    pivot_patterns: "Pivot",
    rsi_extremes: "RSI",
    pattern_recognition: "Pattern",
    bollinger_signals: "Bollinger",
    trend_regime: "Trend",
  ...
  return map[family] || family.replace(/_/g, " ");
}

// ── Small bar: WR vs base ─────────────────────────────────────────────────────

function WrBar({ wr, base }: { wr: number | null | undefined; base: number | null | undefined }) {
  if (wr == null || base == null) return <span className="fin-mut">—</span>;
  const maxPct = 80; // clamp to 80% for display
  const wrPct = Math.min(wr * 100, maxPct);
  const basePct = Math.min(base * 100, maxPct);
  const ahead = wr > base;
  return (
    <div className="tl-wr-bar" title={`WR 21d: ${(wr * 100).toFixed(1)}%  Base: ${(base * 100).toFixed(1)}%`}>
      <div className="tl-wr-track">
        {/* base marker */}
        <div className="tl-wr-base" style={{ left: `${basePct}%` }} />
        {/* actual bar */}
        <div
          className={`tl-wr-fill ${ahead ? "up" : "down"}`}
          style={{ width: `${wrPct}%` }}
        />
      </div>
      <span className={`tl-wr-num ${ahead ? "up" : "down"}`}>{(wr * 100).toFixed(1)}%</span>
    </div>
  );
}
...
function SignalRow({
  ...
  return (
    <div className={`tl-sig-row${firing ? " tl-sig-row--firing" : ""}`}>
      {/* Signal name + family + direction */}
      <div className="tl-sig-head">
        <span className={`tl-dir ${dirClass(dir)}`}>{dirGlyph(dir)}</span>
        <span className="tl-sig-name">{name}</span>
        {family && <span className="tl-family">{family}</span>}
        {firing && daysSinceFire !== null && (
          <span className="tl-days-ago">{daysSinceFire === 0 ? "today" : `${daysSinceFire}d ago`}</span>
        )}
      </div>

      {/* Profile stats (omit when profile unavailable) */}
      {profile && (
        <div className="tl-sig-stats">
          <div className="tl-stat-pair">
            <span className="tl-stat-label">WR 21d vs base</span>
            <WrBar wr={profile.wr_21d} base={profile.base_wr} />
          </div>

          <div className="tl-stat-grid">
            <div className="tl-stat">
              <span className="tl-stat-label">Edge WR</span>
              <span className={`tl-stat-val ${(profile.edge_wr ?? 0) >= 0 ? "up" : "down"}`}>
                {profile.edge_wr != null ? `${(profile.edge_wr * 100) >= 0 ? "+" : ""}${(profile.edge_wr * 100).toFixed(1)}pp` : "—"}
              </span>
            </div>
            <div className="tl-stat">
              <span className="tl-stat-label">MFE/MAE</span>
              <span className="tl-stat-val">{profile.mfe_mae_med != null ? profile.mfe_mae_med.toFixed(2) : "—"}</span>
            </div>
            <div className="tl-stat">
              <span className="tl-stat-label">Fires</span>
              <span className="tl-stat-val">{profile.n_fires != null ? profile.n_fires.toLocaleString() : "—"}</span>
            </div>
            <div className="tl-stat">
              <span className="tl-stat-label">Up-tape %</span>
              <span className="tl-stat-val">{profile.up_tape_pct != null ? (profile.up_tape_pct * 100).toFixed(0) + "%" : "—"}</span>
            </div>
          </div>

          {/* Era split: pre/post 2010 */}
          <div className="tl-era-split">
            {profile.wr_pre2010 != null ? (
              <>
                <span className="tl-era-label">Pre-2010</span>
                <span className="tl-era-val">{fmtWr(profile.wr_pre2010)}</span>
                <span className="tl-era-sep">·</span>
              </>
            ) : (
              <>
                <span className="tl-era-label fin-mut">Pre-2010</span>
                <span className="tl-era-val fin-mut">n/a</span>
                <span className="tl-era-sep">·</span>
              </>
            )}
            <span className="tl-era-label">Post-2010</span>
            <span className="tl-era-val">{fmtWr(profile.wr_post2010)}</span>
            {profile.n_months != null && (
              <span className="tl-era-months fin-mut"> ({profile.n_months}mo)</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="fin-body">
        <div className="fin-empty fin-empty-lg" role="status">
          <span className="fin-empty-title">
            {pick(zh, "Lab data not available for this symbol yet.", "该标的的技术实验室数据暂未就绪。")}
          </span>
        </div>
        <LabFooter />
      </div>
  ...
  return (
    <div className="fin-body tl-panel">
      ...
      <div className="tl-header">
        ...
        {asof && (
          <div className="tl-asof fin-mut">
            {pick(zh, "As of", "截至")} {asof.slice(0, 10)}
          </div>
        )}
      </div>
      ...
      <div className="fin-sec">
        <div className="fin-sec-h">
          {pick(zh, "Firing now", "当前触发")}
          {firing.length > 0 && (
            <span className="tl-firing-count">{firing.length}</span>
          )}
        </div>
      ...
      {other.length > 0 && profiles && (
        <div className="fin-sec">
          ...
          <div className="tl-sig-list tl-sig-list--other">
            {other.map((id) => (
              <SignalRow
                key={id}
                id={id}
