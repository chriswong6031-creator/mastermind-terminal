...
function isoDay(time: string | number): string {
  ...
  const number = typeof time === "number" ? time : Number(time);
  return Number.isFinite(number) ? new Date(number * 1000).toISOString().slice(0, 10) : "";
}

export function parseChartEvents(fund: unknown, preferences: EventPreferences): ChartEvent[] {
  const events: ChartEvent[] = [];
  const root = record(fund);
  const earnings = record(root?.earnings);
  const dividends = record(root?.dividends);
  if (preferences.showEarnings !== false) {
  ...
  if (preferences.showDividends !== false) {
  ...
  if (preferences.showSplits !== false) {
...
/**
 * Convert dated corporate events to lightweight-charts markers. Weekend and
 * holiday events snap to the nearest real bar within five calendar days, so
 * markers never create synthetic x-axis data.
 */
export function buildEventMarkers(
  ...
  if (!days.length) return { markers, byId };
  ...
  for (const event of events) {
    ...
    const config: { color: string; text: ChartEventMarker["text"] } = event.kind === "earnings"
