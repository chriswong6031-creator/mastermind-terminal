...
const LEX: Record<string, [string, string]> = {
  ...
  leadersColdStart: ["Building baseline — publishes after tonight's nightly run / 今晚构建后发布", "基线建立中 — 今晚构建后发布 / Building after tonight's run"],
  leadersStale: ["Snapshot from prior session", "数据来自上一交易日"],
  leadersAbsent: ["Flow Leaders publishes after tonight's build", "今晚构建后发布"],
  leadersAbsentZh: ["今晚构建后发布", "Flow Leaders publishes after tonight's build"],
  leadersCoverage: ["top {n} of {total}", "前 {n} / 共 {total}"],
  leadersSessions: ["sessions {k}/5", "会话 {k}/5"],
  leadersRecurrence: ["recurrence", "重复次数"],
  leadersAccruing: ["accruing", "累积中"],
  leadersAccruingZh: ["累积中", "accruing"],
  leadersNetPrem: ["net prem (~soft)", "净保费（~软性）"],
