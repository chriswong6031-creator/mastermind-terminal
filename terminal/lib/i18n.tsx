...
export const LEX: Record<string, [string, string]> = {
  ...
  // Options workflow — one guided path across the already-live truth surfaces.
  ...
  optionsWorkflowTitle: ["From tape to trigger", "从盘口到提醒"],
  optionsWorkflowSubtitle: [
    "Four checkpoints through the live desk. Each stop opens its source surface; none of them promotes a trade.",
    "沿实时工作台完成四个检查点。每一步都打开对应的数据源页面；任何一步都不会把观察升级为交易建议。",
  ],
  optionsWorkflowStepsAria: ["Options workflow checkpoints", "期权工作流程检查点"],
  optionsWorkflowCurrent: ["CURRENT VIEW", "当前视图"],
  optionsWorkflowReopenCta: ["Open view", "打开视图"],
  optionsWorkflowTapeTitle: ["Read what traded", "先看实际成交"],
  optionsWorkflowTapeBody: [
    "Check snapshot and source age first, then scan premium magnitude and repeat activity. ~Side remains heuristic.",
    "先检查快照与源数据时间，再查看权利金规模和重复活动。~方向仍是启发式推断。",
  ],
  optionsWorkflowTapeReceipt: ["Live Flow · covered options tape", "实时资金流 · 覆盖范围内的期权盘口"],
  optionsWorkflowTapeCta: ["Open live tape", "打开实时盘口"],
  optionsWorkflowStructureTitle: ["Test the structure", "检验仓位结构"],
  optionsWorkflowStructureBody: [
    "Compare the tape with GEX walls, the gamma flip, and positioning. Dealer sign is a convention, not an observed fact.",
    "用 GEX 墙、伽马翻转位和持仓结构核对盘口。做市商符号是惯例假设，不是直接观测事实。",
  ],
  optionsWorkflowStructureReceipt: ["GEX · standard dealer convention", "GEX · 标准做市商惯例"],
  optionsWorkflowStructureCta: ["Open exposure", "打开敞口"],
  optionsWorkflowPlanTitle: ["Open the plan", "打开计划"],
  optionsWorkflowPlanBody: [
    "Use a published Prophet plan only when it names the contract and carries a current or EOD mark. Options remain context overlays, not signal inputs.",
    "仅在已发布的 Prophet 计划明确合约并带有当前或收盘标记时使用。期权仍是背景叠加信息，不参与信号生成。",
  ],
  optionsWorkflowPlanReceipt: ["Prophet plans · display-only", "Prophet 计划 · 仅供展示"],
  optionsWorkflowPlanCta: ["Open Prophet plans", "打开 Prophet 计划"],
  optionsWorkflowAlertTitle: ["Arm the condition", "设置触发条件"],
  optionsWorkflowAlertBody: [
    "Choose a covered options condition. It fires only on current evaluable evidence, then disarms until you re-arm it.",
    "选择覆盖范围内的期权条件。仅在当前证据可评估时触发，随后暂停，直至你重新启用。",
  ],
  optionsWorkflowAlertReceipt: ["Alert Center · fresh-source gate", "提醒中心 · 新鲜数据门槛"],
  optionsWorkflowAlertCta: ["Arm an options alert", "设置期权提醒"],
