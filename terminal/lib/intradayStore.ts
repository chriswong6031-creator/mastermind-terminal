...
export async function withStoredHistory(sym: string, tf: string, ext: boolean, live: Bar6[]): Promise<Bar6[]> {
  ...
  if (!base) return live;
  // Prefer five-minute history for regular hourly charts. It can be cleanly
  // filtered at 09:30 before resampling; a provider-built 09:00 hourly bar has
  // already mixed 30 minutes of premarket data and cannot be repaired.
  let stored: Bar6[] = [];
