...
def test_no_artifact_emits_exactly_what_the_pre_fence_engine_emitted(tmp_path):
    """THE SCORED FALLBACK, proved by golden diff.

    ``tests/golden/no_state_slice.json`` was generated by the emitter at origin/master
    @ 397700aa. New BOTTOM_WATCH / stop-sweep events are deliberately display-only, so
    remove those additive rows and compare every legacy event plus every scored state
    field byte-for-byte. The display tail is allowed to change ``last_signal`` and
    ``bars_since_signal``; it must never change the scored walk.
    """
    ...
    def assert_scored_fallback(actual):
        legacy = [s for s in actual["signals"]
                  if s.get("type") != "BOTTOM_WATCH"
                  and s.get("quality") != "stop_sweep_reclaim"]
        assert legacy == golden["signals"]
        display_tail = {"last_signal", "bars_since_signal"}
        assert {k: v for k, v in actual["state"].items() if k not in display_tail} == {
            k: v for k, v in golden["state"].items() if k not in display_tail
        }

    assert_scored_fallback(tape.emit(symbol="SYNTH", override_gate=g))
    assert_scored_fallback(tape.emit())  # no gate at all (caller has not wired one)
